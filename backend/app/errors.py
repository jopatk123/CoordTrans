...
class ApiResponse(BaseModel):
    ...
    def not_found(cls, msg: str = "未找到结果") -> dict:
        """未找到响应"""
        return cls(status="error", code=404, msg=msg, data=None).model_dump()


class AppError(Exception):
    """应用自定义错误基类"""

    def __init__(self, msg: str, code: int = 400, detail: Optional[str] = None):
        self.msg = msg
        self.code = code
...
async def app_error_handler(request: Request, exc: AppError) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=exc.code, content=ApiResponse.error(msg=exc.msg, code=exc.code)
    )
...
async def validation_exception_handler(
    ...
    return JSONResponse(
        status_code=422, content=ApiResponse.error(msg=detail, code=422)
    )
...
async def generic_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=500,
        content=ApiResponse.error(msg="服务器内部错误，请稍后重试", code=500),
    )
