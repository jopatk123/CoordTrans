...
async def read_upload_file(file: UploadFile) -> Tuple[pd.DataFrame, bytes]:
    ...
    if not any(filename_lower.endswith(ext) for ext in ALLOWED_EXTENSIONS):
        raise HTTPException(
            status_code=400, detail="不支持的文件格式，请上传 CSV 或 Excel 文件"
        )
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"读取文件失败: {str(e)[:100]}")
...
def extract_addresses(df: pd.DataFrame, column: str, max_length: int) -> List[str]:
    """
...
def extract_locations(
    ...
    if all(loc == "" for loc in locations):
        raise HTTPException(
            status_code=400, detail="所有经纬度数据无效，请检查文件格式"
        )
...
def _sanitize_cell(val: Any) -> Any:
    """对可能触发 Excel 公式注入的字符串添加单引号前缀。

    以 = + - @ \\t \\r 开头的字符串会被 Excel 解析为公式，
    前置 ' 可强制 Excel 将其视为纯文本。
    """
...
def create_excel_response(df: pd.DataFrame, filename: str) -> StreamingResponse:
    ...
    """
    output = io.BytesIO()
    # 转义公式注入字符，防止导出文件在 Excel 中执行公式
    df.map(_sanitize_cell).to_excel(output, index=False)
    output.seek(0)
